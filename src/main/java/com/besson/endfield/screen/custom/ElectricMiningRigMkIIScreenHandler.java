...
public class ElectricMiningRigMkIIScreenHandler extends AbstractContainerMenu {
    ...
    public ElectricMiningRigMkIIScreenHandler(int sync, Inventory playerInventory, FriendlyByteBuf packetByteBuf) {
        this(sync, playerInventory, playerInventory.player.level().getBlockEntity(packetByteBuf.readBlockPos()),
                new SimpleContainerData(2));
    }
    public ElectricMiningRigMkIIScreenHandler(int syncId, Inventory playerInventory, BlockEntity blockEntity, ContainerData propertyDelegate) {
        super(ModScreens.ELECTRIC_MINING_RIG_MK_II_SCREEN.get(), syncId);
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, entity.getBlockPos()),
                pPlayer, ModBlocks.ELECTRIC_MINING_RIG_MK_II.get());
    }
