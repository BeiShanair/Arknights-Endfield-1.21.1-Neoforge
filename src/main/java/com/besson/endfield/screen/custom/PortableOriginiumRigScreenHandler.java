...
public class PortableOriginiumRigScreenHandler extends AbstractContainerMenu {
    ...
    public PortableOriginiumRigScreenHandler(int syncId, Inventory playerInventory, FriendlyByteBuf buf) {
        this(syncId, playerInventory, playerInventory.player.level().getBlockEntity(buf.readBlockPos()), new SimpleContainerData(2));
    }

    public PortableOriginiumRigScreenHandler(int syncId, Inventory playerInventory, BlockEntity blockEntity, ContainerData propertyDelegate) {
        super(ModScreens.PORTABLE_ORIGINIUM_RIG_SCREEN.get(), syncId);
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, entity.getBlockPos()),
                pPlayer, ModBlocks.PORTABLE_ORIGINIUM_RIG.get());
    }
}
