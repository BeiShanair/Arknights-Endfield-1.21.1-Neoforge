...
public class LNTowerBlockRenderer extends GeoBlockRenderer<LNTowerBlockEntity> {
    public LNTowerBlockRenderer(BlockEntityRendererProvider.Context context) {
        super(new LNTowerModel());
    }

    @Override
    public void actuallyRender(PoseStack poseStack, LNTowerBlockEntity animatable, BakedGeoModel model, @Nullable RenderType renderType, MultiBufferSource bufferSource, @Nullable VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, int colour) {
