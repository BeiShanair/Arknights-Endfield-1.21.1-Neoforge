...
public class GunTowerBlockRenderer extends GeoBlockRenderer<GunTowerBlockEntity> {
    public GunTowerBlockRenderer(BlockEntityRendererProvider.Context context) {
        super(new GunTowerBlockModel());
    }

    @Override
    public void actuallyRender(PoseStack poseStack, GunTowerBlockEntity animatable, BakedGeoModel model, @Nullable RenderType renderType, MultiBufferSource bufferSource, @Nullable VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, int colour) {
