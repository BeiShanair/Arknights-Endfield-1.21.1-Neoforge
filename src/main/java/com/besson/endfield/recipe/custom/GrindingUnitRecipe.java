...
public record GrindingUnitRecipe(NonNullList<InputEntry> input, ItemStack output) implements Recipe<DoubleRecipeInput> {
    ...
    public RecipeSerializer<?> getSerializer() {
        return ModRecipes.GRINDING_UNIT_SERIALIZER.get();
    }

    @Override
    public RecipeType<?> getType() {
        return ModRecipes.GRINDING_UNIT_TYPE.get();
    }
    ...
    public static class Serializer implements RecipeSerializer<GrindingUnitRecipe> {
        public static final MapCodec<GrindingUnitRecipe> CODEC = RecordCodecBuilder.mapCodec(instance ->
                instance.group(Codec.list(InputEntry.CODEC.codec()).fieldOf("input").forGetter(GrindingUnitRecipe::input),
                                ItemStack.CODEC.fieldOf("output").forGetter(GrindingUnitRecipe::output))
                        .apply(instance, (list, out) -> {
                            NonNullList<InputEntry> inputs = NonNullList.of(InputEntry.EMPTY, list.toArray(new InputEntry[0]));
                            return new GrindingUnitRecipe(inputs, out);
                        }));

        public static final StreamCodec<RegistryFriendlyByteBuf, GrindingUnitRecipe> STREAM_CODEC =
                StreamCodec.of((buf, recipe) -> {
                    ...
                    return new GrindingUnitRecipe(nn, out);
                });

        @Override
        public MapCodec<GrindingUnitRecipe> codec() {
            return CODEC;
        }

        @Override
        public StreamCodec<RegistryFriendlyByteBuf, GrindingUnitRecipe> streamCodec() {
            return STREAM_CODEC;
        }
    }
}
