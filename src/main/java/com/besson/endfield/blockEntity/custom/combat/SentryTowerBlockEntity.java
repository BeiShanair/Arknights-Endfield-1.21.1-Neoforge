...
public class SentryTowerBlockEntity extends BaseGunTower {
    public SentryTowerBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.SENTRY_TOWER.get(), pos, state);
    }
    ...
    protected void shoot(Level world) {
        ...
        Vec3 targetPos = t.position().add(0, t.getBbHeight() * 0.8, 0);

        spawnBulletTrail(world, towerPos, targetPos);
        t.invulnerableTime = 0;
        boolean died = t.hurt(world.damageSources().generic(), 116.0f);
    ...
    protected static void spawnBulletTrail(Level level, Vec3 start, Vec3 end) {
        ...
        for (int i = 0; i < steps; i++) {
            ((ServerLevel) level).sendParticles(ParticleTypes.INSTANT_EFFECT, pos.x, pos.y, pos.z, 1, 0, 0, 0, 0);
            pos = pos.add(step);
        }
    }
}
