...
public class MarshGasMkIBlockEntity extends BaseGunTower {
    public MarshGasMkIBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.MARSH_GAS_MK_I.get(), pos, state);
    }
    ...
    protected void shoot(Level world) {
        ...
        boolean died = t.hurt(world.damageSources().generic(), 54.0f);
    ...
    protected static void spawnBulletTrail(Level level, Vec3 start, Vec3 end) {
        ...
        for (int i = 0; i < steps; i++) {
            ((ServerLevel) level).sendParticles(ParticleTypes.DRAGON_BREATH, pos.x, pos.y, pos.z, 1, 0, 0, 0, 0);
            pos = pos.add(step);
        }
    }
